-- =====================================================================
-- Kidoo — o estabelecimento que saiu.
--
-- Para um banco que JÁ EXISTE, e DEPOIS do `19-endereco-e-telefone.sql`.
-- Cole inteiro no SQL Editor do Supabase e clique em Run. É idempotente.
--
-- O que entra:
--   1. `partners.active`. Desligado some do catálogo e para de receber
--      reserva; o histórico de quem já foi continua inteiro. Não se apaga
--      parceiro: `bookings.activity_id` é `on delete restrict`, e o banco
--      recusa apagar quem já recebeu criança — com razão.
--   2. `book_session` passa a recusar reserva em estabelecimento desligado
--      E em atividade despublicada. A segunda era uma porta aberta desde que
--      "despublicar" existe: o catálogo escondia, mas a reserva chega por um
--      id de turma, e id continua valendo depois de a tela ter sumido.
--   3. Duas funções para quem analisa: listar e ligar/desligar.
--
-- Depois de rodar, o painel de quem analisa ganha a aba "Estabelecimentos".
-- =====================================================================
...
-- A coluna existe, e ninguém saiu do ar sem querer?
select count(*) filter (where active)     as no_ar,
       count(*) filter (where not active) as fora
  from partners;

-- As duas funções novas estão lá?
select proname
  from pg_proc
 where proname in ('set_partner_active', 'admin_partners')
 order by proname;

-- E `book_session` ficou com TODAS as recusas? A lista tem de conter
-- `already_booked` — ela veio de outra migration, e uma função reescrita
-- perde o que não for repetido.
select unnest(array['already_booked','partner_inactive','activity_inactive','session_full'])
         as recusa,
       prosrc like '%' || unnest(array['already_booked','partner_inactive','activity_inactive','session_full']) || '%'
         as presente
