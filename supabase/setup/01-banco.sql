...
-- =====================================================================
-- 20260101000020_portao_da_assinatura.sql
-- =====================================================================
...
-- =====================================================================
-- 20260101000025_email_da_decisao.sql
-- =====================================================================

-- =====================================================================
-- O candidato fica sabendo da decisão.
-- =====================================================================
--
-- Aprovar e recusar mexiam no banco e pronto. O estabelecimento aprovado
-- descobria entrando no painel por conta própria; o recusado só lia o motivo
-- se voltasse sozinho. A tela de recusa foi construída para ele corrigir e
-- reenviar — e a mensagem ficava esperando que ele adivinhasse que devia
-- voltar.
--
-- Caixa de saída, e não chamada HTTP dentro da função, pelo mesmo motivo dos
-- avisos de vaga: `approve_application` cria parceiro, vínculo de dono,
-- atividades e repasse numa transação só. Uma ida à rede ali seguraria essa
-- transação pelo tempo do serviço de e-mail responder, e uma queda dele
-- desfaria uma aprovação que já estava certa. Escrever uma linha é instantâneo
-- e não falha por rede; entregar pode tentar de novo amanhã sem desfazer nada.
--
-- A caixa guarda O QUE ACONTECEU, não o e-mail pronto. Assim mudar o texto não
-- é migration, e uma mensagem que ficou na fila sai com o texto novo. Quem
-- monta o HTML é a Edge Function `enviar-emails`.
-- =====================================================================
...
-- =====================================================================
-- 20260101000026_vencida_volta_para_a_fila.sql
-- =====================================================================
...
-- =====================================================================
-- 20260101000027_excluir_a_conta.sql
-- =====================================================================
...
-- =====================================================================
-- 20260101000028_capa_passa_por_analise.sql
-- =====================================================================
...
-- =====================================================================
-- 20260101000029_denuncia.sql
-- =====================================================================

-- =====================================================================
-- A família consegue denunciar uma imagem imprópria.
-- =====================================================================
--
-- A 000028 pôs a capa nova numa fila antes de ir ao ar. Isto cobre o que
-- escapa da fila: a capa que já estava publicada antes dela existir, e o
-- julgamento que erra — quem analisa é gente, e vê uma imagem por segundo.
--
-- ------------------------------------------------ a decisão que pesa aqui --
--
-- UMA denúncia já tira a capa do ar, sem esperar uma segunda.
--
-- É abusável de propósito, e a conta é assimétrica: uma capa escondida por
-- engano vira a foto genérica da modalidade por alguns minutos, e volta com
-- um clique. Uma imagem imprópria que fica no ar está num app de criança, e
-- não volta atrás. Entre punir o parceiro por engano e deixar a imagem
-- passar, a escolha é a primeira.
--
-- O abuso fica limitado por construção: uma denúncia por família por
-- atividade (índice único), e a capa escondida volta inteira, porque a URL
-- antiga é guardada na própria denúncia.
--
-- Esconder é só para imagem. Denúncia sobre descrição ou segurança entra na
-- fila e espera gente — não há nada para tirar do ar automaticamente, e
-- apagar texto de parceiro por um clique de terceiro seria outra história.
-- =====================================================================
...
-- =====================================================================
-- 20260101000030_votacao.sql
-- =====================================================================

-- =====================================================================
-- Votação com fotos: inscrição, cédula e apuração.
-- =====================================================================
--
-- Nada aqui sabe o que está sendo votado. `polls`, `poll_categories`,
-- `poll_entries` e `ballots` servem para fantasia de Halloween do mesmo jeito
-- que para melhor estabelecimento parceiro do ano — a diferença mora nos
-- rótulos que alguém digita, não no schema.
--
-- É de propósito: a primeira votação é de uma festa, e seria fácil escrever
-- `costume_id` e `scariest_votes` em tudo. Aí a segunda votação começaria com
-- uma migration de renomeação ou com nomes mentindo para sempre.
--
-- ------------------------------------------------------ quem vota é anônimo --
--
-- Convidado de festa não cria conta. Quem vota é `anon`, identificado por uma
-- chave que o navegador guarda — e a garantia de um voto por categoria é
-- índice único no banco, não disciplina da tela. Recarregar, voltar e clicar
-- duas vezes não passam.
--
-- Não impede quem abrir uma janela anônima de propósito. Nada impede, sem
-- entregar um código na mão de cada pessoa — e é por isso que `voter_key` é
-- texto livre: o dia em que a votação valer algo, os códigos são gerados e
-- distribuídos, e nada aqui muda.
-- =====================================================================
...
-- =====================================================================
-- 20260101000031_numero_do_votante.sql
-- =====================================================================

-- =====================================================================
-- O papel entregue na porta passa a ser a identidade de quem vota.
-- =====================================================================
--
-- Até aqui a chave do votante era um id que o navegador guardava. Ele
-- resolve o clique repetido e o F5, e não resolve o que interessa numa festa:
-- saber QUEM votou, e garantir que cada convidado vote uma vez só.
--
-- Agora quem organiza escolhe quantos papéis vai entregar — 10 convidados, 60
-- convidados — e os números são de 1 até esse total. Na hora de votar, a
-- pessoa digita o número que recebeu. É ele que vai para `voter_key`, e o
-- índice único da 000030 (`category_id, voter_key`) passa a valer por PAPEL,
-- não por aparelho.
--
-- Duas consequências que vêm de graça:
--
--   * o mesmo celular serve a festa inteira — a avó vota com o papel dela no
--     telefone do neto, e continua sendo um voto por papel;
--   * quem organiza vê quais números já votaram, que é a pergunta que fez o
--     papel existir.
--
-- `voter_numbers` nulo mantém o jeito antigo, e é o que uma votação sem porta
-- física precisa: parceiro do ano não tem como receber papel na mão.
--
-- ------------------------------------------------------- o que isto NÃO é --
--
-- Número sequencial não é segredo. Quem tem o link e sabe que a festa tem 60
-- convidados pode digitar 37 e votar no lugar de quem ainda não votou. A senha
-- da festa é o que fecha essa porta, e é por isso que a tela passa a pedi-la
-- junto com o número. Contra um convidado mal-intencionado DENTRO da festa,
-- nada disso adianta — e nenhum sistema de votação de festa adianta.
-- =====================================================================
...
-- =====================================================================
-- 20260101000032_palavra_no_papel.sql
-- =====================================================================
