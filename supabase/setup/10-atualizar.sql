...
--   000015  cadastro do parceiro: pedido, análise e aprovação
--
...
begin
  ...
  insert into kidoo_status values
    (10, 'partner_agenda com o estabelecimento',
     case when exists (select 1 from information_schema.routines r
                         join information_schema.parameters p on p.specific_name = r.specific_name
  ...
  insert into kidoo_status values
    (12, 'cadastro de parceiro (pedido e aprovação)',
     case when to_regclass('public.partner_applications') is null then 'FALTANDO'
          when not exists (select 1 from kidoo_admins) then 'SEM NINGUÉM PARA APROVAR'
          else 'ok' end);
