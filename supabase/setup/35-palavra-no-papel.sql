-- =====================================================================
-- Kidoo — o papel da porta passa a trazer uma palavra sorteada.
--
-- Para um banco que JÁ EXISTE, e DEPOIS da 34. Cole inteiro no SQL Editor do
-- Supabase e clique em Run. É idempotente.
--
-- A 34 entregou papéis numerados de 1 até o total, e o cabeçalho dela dizia o
-- que eles não resolviam: número sequencial não é segredo. Quem tem o link e
-- sabe que a festa tem 60 convidados digita 37 e vota no lugar de quem ainda
-- não votou.
--
-- Agora cada papel recebe um código sorteado — `MORCEGO 84` —, e é ele que a
-- pessoa digita. São 160 palavras vezes 90 duplas de dígitos, e só valem os
-- papéis entregues.
--
-- O que NÃO muda: `voter_key` continua sendo o número do papel. O código é a
-- prova de que aquele papel é seu, não a identidade — o índice único, a grade
-- de quem organiza e a apuração seguem iguais.
--
-- As votações criadas ANTES disto continuam valendo pelo número: papel
-- entregue não se recolhe.
--
-- Sem acento e sem espaço no que se compara: o papel mostra `ABÓBORA 12` e
-- quem digita "abobora12" ou "ABOBORA-12" entra do mesmo jeito.
-- =====================================================================
...
-- A lista de códigos é fechada: quem vota não lê os papéis dos outros.
select relrowsecurity as rls_ligada,
       has_table_privilege('anon', 'poll_tickets', 'select') as anon_le_NAO_PODE,
       has_table_privilege('authenticated', 'poll_tickets', 'select') as painel_le_direto_NAO_PODE
  from pg_class where oid = 'poll_tickets'::regclass;

-- E o normalizador faz o que promete: os três viram a mesma coisa.
select poll_word('ABÓBORA 12') as a, poll_word('abobora-12') as b, poll_word(' Abóbora12 ') as c;
