...
ok(
  terQui.every((d) => d.getDay() === TER || d.getDay() === QUI),
  'e nenhuma cai num dia que ele não pediu',
);
ok(
  terQui.every((d) => d.getHours() === 18 && d.getMinutes() === 0),
  'todas às 18h em ponto, no relógio do balcão',
);

const tempos = terQui.map((d) => d.getTime());
ok(
  tempos.every((t, i) => i === 0 || t > tempos[i - 1]!),
  'as datas vêm em ordem',
);
...
ok(
  cedo.every((d) => d.getTime() > AGORA.getTime()),
  'e nenhuma data no passado escapa',
);
...
ok(
  datasDaSerie({ diasDaSemana: [], hora: '18:00', semanas: 8 }, AGORA).length === 0,
  'sem dia marcado não há série',
);
ok(
  datasDaSerie({ diasDaSemana: [TER], hora: '18:00', semanas: 0 }, AGORA).length === 0,
  'sem semana não há série',
);
ok(
  datasDaSerie({ diasDaSemana: [TER], hora: '', semanas: 8 }, AGORA).length === 0,
  'campo de hora vazio não vira meia-noite escondida',
);
ok(
  datasDaSerie({ diasDaSemana: [TER], hora: 'seis da tarde', semanas: 8 }, AGORA).length === 0,
  'hora que não é hora também não',
);

// ---- o resumo que ele confere antes de clicar -------------------------------
ok(
  resumoDaSerie(terQui) === 'ter, 10/03 · qui, 12/03 · ter, 17/03 · +13',
  'o resumo mostra as três primeiras e conta o resto',
);
ok(
  resumoDaSerie(terQui.slice(0, 2)) === 'ter, 10/03 · qui, 12/03',
